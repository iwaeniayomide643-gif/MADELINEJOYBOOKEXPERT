...
async function run() {
  for (const v of videos) {
    ...
    try {
      ...
      // Look for filename or extra text in body
      const docNameMatch = previewRes.body.match(/"title":"([^"]+)"/) || viewRes.body.match(/"title":"([^"]+)"/);
      if (docNameMatch) {
        console.log(`JSON Title: ${docNameMatch[1]}`);
      }
    } catch (e) {
